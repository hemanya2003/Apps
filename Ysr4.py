...
tile_layer_terrain_rgb = pdk.Layer(
    'TileLayer',
    data=None,
    get_tile_data=f'https://api.maptiler.com/tiles/terrain-rgb-v2/{{z}}/{{x}}/{{y}}.png?key={MAPTILER_API_KEY}',
    pickable=False,
    tile_size=256,
)
...
# Base map selection
base_map = st.selectbox("Select Base Map", ["Satellite", "Topo v2", "Terrain RGB"])

# Map style selection
map_style = st.selectbox(
    "Select Map Style", 
    ["Hybrid", "Streets", "Outdoor", "Topo Map", "Landscape", "Terrain Quantized Mesh"]
)
...
map_styles = {
    ...
    "Terrain Quantized Mesh": f"https://api.maptiler.com/tiles/terrain-quantized-mesh-v2/tiles.json?key={MAPTILER_API_KEY}"
}

# Select base layer
if base_map == "Satellite":
    tile_layer = tile_layer_satellite
elif base_map == "Topo v2":
    tile_layer = tile_layer_topo
elif base_map == "Terrain RGB":
    tile_layer = tile_layer_terrain_rgb
...
deck = pdk.Deck(
    ...
    map_style=map_styles[map_style],  # Dynamically selecting map style
)
