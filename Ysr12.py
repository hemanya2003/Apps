...
# Load shapefile
shapefile_path = "LUSE_FINAL.shp"  # Replace with your correct path
...
swipe_position = st.slider("Swipe Effect (0% - Full Left, 100% - Full Right)", 0, 100, 50)

# Adjust opacity dynamically for land use layer
left_opacity = (100 - swipe_position) / 100
right_opacity = swipe_position / 100

# Create terrain layer (not affected by swipe)
...
# Create land use polygon layers with swipe effect
polygon_layer_left = pdk.Layer(
    ...
    elevation_scale=1,
    pickable=True,
    opacity=left_opacity,  # Dynamic opacity for swipe effect
)
...
polygon_layer_right = pdk.Layer(
    ...
    elevation_scale=1,
    pickable=True,
    opacity=right_opacity,  # Dynamic opacity for swipe effect
)

# Define layers based on user selection
layers = []
if terrain_enabled:
    layers.append(terrain_layer)  # Terrain is not affected by swipe
if show_landuse:
...
st.write(f"Left Layer Opacity: {left_opacity}")
st.write(f"Right Layer Opacity: {right_opacity}")