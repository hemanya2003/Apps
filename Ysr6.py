...
# MapTiler Styles (with terrain style URL directly in map style)
map_styles = {
    ...
    "Terrain": f"https://api.maptiler.com/maps/terrain-rgb-v2/style.json?key={MAPTILER_API_KEY}",  # Using terrain-rgb-v2 directly
}
...
swipe_layer = pdk.Layer(
    ...
    elevation_scale=1,
    pickable=True,
    visible=show_landuse,  # Visibility controlled by checkbox
    opacity=swipe_position / 100,  # Swipe effect controlled by slider
)

# Create PyDeck Map Layers
layers = []
if show_landuse:  # Only add the swipe layer if "Show Land Use Polygons" is checked
