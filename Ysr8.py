...
# Swipe Position Slider
swipe_position = st.slider("Swipe Position", 0, 100, 50)

# Create PyDeck map with the selected style and layers
layers = []

# Add TileLayer for Terrain
terrain_layer = pdk.Layer(
    "TileLayer",
    data=None,  # No data for tile layer, just the URL
    tile_size=256,
    get_tile_url=map_styles["Terrain"],  # URL for terrain tile layer
    opacity=0.7,
)

# Add polygon layer if checkbox is checked
if show_landuse:
    layers.append(polygon_layer)

# Add terrain layer to layers
layers.append(terrain_layer)

# Define the Deck with the swipeable map layer
deck = pdk.Deck(
    layers=layers,
    initial_view_state=view_state,
    map_style=map_styles[map_style],  # Apply selected MapTiler style
    swipe=swipe_position,  # Apply swipe position
)
